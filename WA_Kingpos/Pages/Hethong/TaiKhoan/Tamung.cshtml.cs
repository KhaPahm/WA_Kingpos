...
{
    [Authorize]
    public class TamungModel : PageModel
    {
        ...
        public IActionResult OnGet(string id)
        {
            ...
            {
                cls_Main.WriterLog(cls_Main.sFilePath, "SYS_USER Update TamUng : " + id, ex.ToString(), "0");
        ...
        public IActionResult Onpost(string id, clsTaikhoan item)
        {
            ...
            {
                string sConnectionString_live = cls_ConnectDB.GetConnect("0");
                if (item.tientamung == null || item.tientamung == "")
                    item.tientamung = "0";
                string sSQL = "";
                sSQL += "Update SYS_USER set DateTime_Log=GETDATE(), TamUng="+cls_Main.SQLString(item.tientamung) + "\n";
