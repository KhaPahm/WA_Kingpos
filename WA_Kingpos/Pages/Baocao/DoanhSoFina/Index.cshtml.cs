...
namespace WA_Kingpos.Pages.Baocao.DoanhSoFina
{
    public class IndexModel : PageModel
    {
        public List<ThongTinDoanhSo> listitem { set; get; } = new();
        ...
        public List<clsKho> listKho { get; set; } = new();

        [BindProperty]
        public string sNgay { get; set; } = "";

        [BindProperty]
        public string sKho { get; set; } = "0";

        public IActionResult OnGet(string id1, string id2, string id3, string id4)
        {
            try
            {
                //check quyen
                if (!cls_UserManagement.AllowView("23120905", HttpContext.Session.GetString("Permission")))
                {
                    return RedirectToPage("/AccessDenied");
                }
                //xu ly trang
                LoadData(id1 ?? MaNhanVien, id2 ?? sNgay, id3 ?? sKho, id4 ?? strNhomNhanVien);
                return Page();
            }
            catch (Exception ex)
            {
        ...
        public IActionResult OnPost(string submitButton, [FromForm(Name = "MaNhanVien")] List<string> id1, [FromForm(Name = "sNgay")] string id2, [FromForm(Name = "sKho")] string id3, [FromForm(Name = "strNhomNhanVien")] string id4)
        {
            if (submitButton == "view")
            {
                return RedirectToPage("Index", new { id1 = string.Join(",", id1), id2 = id2, id3 = id3, id4 = id4 });
            }
            else if (submitButton == "print")
            {
                ...
                string reportPath = Path.Combine(cls_Main.sFilePathReport, "rptDoanhSo.frx"); // rptDSHanghoa rptDoanhSo
                webReport.Report.Load(reportPath);
                LoadData(string.Join(",", id1), id2, id3, id4);
                ...
                webReport.Report.SetParameterValue("sKho", listKho.FirstOrDefault(r => r.ma_kho == sKho)?.ten_kho);
                webReport.Report.SetParameterValue("sNgay", sNgay);
                webReport.Report.SetParameterValue("sumThanhTien", listitem.Sum(r => r.THANHTIEN));
                webReport.Report.SetParameterValue("sumChenhLech", listitem.Sum(r => r.CHENHLECH_THANHTIEN));
                webReport.Report.SetParameterValue("sumSLQuyDoi", listitem.Sum(r => r.DOANHSO_KPI));
                ...
                stream.Position = 0;
                return File(stream, "application/pdf", "rptDoanhSo.pdf");
            }
            else if (submitButton == "excel")
            {
                ...
                string tieude = "BÁO CÁO DOANH SỐ";
                // Tạo một package ExcelPackage từ EPPlus
                using (ExcelPackage package = new ExcelPackage())
                {
                    // Tạo một worksheet trong package
                    ExcelWorksheet worksheet = package.Workbook.Worksheets.Add("rptDoanhSo");
                    ...
                    worksheet.Cells["A6"].Value = "Kho:";
                    worksheet.Cells["B6"].Style.Font.Bold = true;
                    worksheet.Cells["B6"].Value = listKho.FirstOrDefault(r => r.ma_kho == sKho)?.ten_kho;

                    worksheet.Row(8).Style.Font.Bold = true;
                    // header table
                    worksheet.Cells["A8"].LoadFromArrays(new List<string[]>{
                        new[] { "Tên đại lý", "Phiếu đặt hàng", "Ngày thanh toán","Mặt hàng", "ĐVT", "Số lượng",
                    "Đơn giá", "Thành tiền", "Giá bảng kê", "Chênh lệch thành tiền", "Kho", "HS quy đổi", "SL quy đổi (Kg)"} });
                    worksheet.Row(8).Style.WrapText = true;

                    // Đưa dữ liệu từ DataTable vào worksheet
                    worksheet.Cells["A9"].LoadFromArrays(listitem.Select(r => new object?[] { r.TENKHACHHANG, r.PHIEUDENGHI, r.NGAYTHANHTOAN,
                        r.TEN_HANGHOA,r.TEN_DONVITINH,r.SOLUONG, r.DONGIA,r.THANHTIEN, r.DONGIA_BANGKE,r.CHENHLECH_THANHTIEN,r.TEN_KHO,r.HESOKPI,r.DOANHSO_KPI }).ToList());

                    worksheet.Columns[3].Style.Numberformat.Format = "dd/mm/yyyy hh:mm AM/PM";
                    ...
                    stream.Position = 0;
                    return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "rptDoanhso.xlsx");
                }
            }
            return Page();
        }
        private void LoadData(string id1, string id2, string id3, string id4)
        {
            ...
            sSQL = "SELECT * FROM dbo.KHO WHERE SUDUNG = 1 ORDER BY TEN_KHO";
            var dtKho = cls_Main.ReturnDataTable_NoLock(sSQL, sConnectionString_live);
            listKho = dtKho.ToList<clsKho>();
            listKho.Insert(0, new clsKho() { ten_kho = "Tất cả", ma_kho = "0" });
            ...
            sSQL = $"SELECT * FROM dbo.fn_Ct_DoanhSoNhanVien({cls_Main.SQLString(MaNhanVien)} , 0, {cls_Main.SQLString(ngay_bd)}, {cls_Main.SQLString(ngay_kt)}, {cls_Main.SQLString(sKho)})";
            TableMain = cls_Main.ReturnDataTable_NoLock(sSQL, sConnectionString_live);
            listitem = TableMain.ToList<ThongTinDoanhSo>();
