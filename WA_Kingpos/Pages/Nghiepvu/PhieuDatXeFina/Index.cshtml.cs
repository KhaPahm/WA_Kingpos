...
namespace WA_Kingpos.Pages.Nghiepvu.PhieuDatXeFina
{
    ...
    public class IndexModel : PageModel
    {
        ...
        public IActionResult OnGet(string tx, string k, string n, string bs)
        {
            try
            {
                //check quyen
                if (!cls_UserManagement.AllowAdd("14102310", HttpContext.Session.GetString("Permission")))
                {
            ...
            {
                cls_Main.WriterLog(cls_Main.sFilePath, "PhieuDatXeFina Add", ex.ToString(), "0");
        ...
        public IActionResult OnPost(string submitButton)
        {
            try
            {
                ...
                {
                    return RedirectToPage("Index", new { tx = PHIEUDATXE.MADOITUONG, k = PHIEUDATXE.MA_KHO, n = PHIEUDATXE.NGAYGIAOHANG, bs = PHIEUDATXE.BIENSOXE });
                }
                if (submitButton == "btn3Create")
                {
                    ...
                    else
                    {
                        ...
                        sSQL = "select MAPDNDC from PHIEUDATXE where MAPDNDC=" + cls_Main.SQLString(PHIEUDATXE.MAPDNDC);
                        DataTable dt = cls_Main.ReturnDataTable_NoLock(sSQL, sConnectionString_live);
                        if (dt.Rows.Count > 0)
                        {
                            sSQL = "SELECT [dbo].[fc_NewCode_PHIEUDATXE]()";
        ...
        private void LoadData(string tx = "", string k = "0", string n = "", string bs = "")
        {
            //SoPhieu
            string sConnectionString_live = cls_ConnectDB.GetConnect("0");
            ...
            PHIEUDATXE.NGAYGIAOHANG = DateTime.Now.ToString("dd/MM/yyyy");
            //List Phieu dat hang
            string nguoitao = User.FindFirst("Username")?.Value;
            string manhanvien = User.FindFirst("UserId")?.Value;
            sSQL = "EXEC SP_FINA_SELECT_DATA @LOAI=49, @USERLOGIN =" + cls_Main.SQLString(manhanvien) + ",@MA_KHO=" + cls_Main.SQLString(k);
            dt = cls_Main.ReturnDataTable_NoLock(sSQL, sConnectionString_live);
